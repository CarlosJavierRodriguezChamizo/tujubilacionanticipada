...
## 2026-09-03 — seo-020 Añadir a /simulador el enlace al simulador oficial de la Seguridad Social y las limitaciones de la estimación (seo)
- Archivos: src/pages/simulador.astro
- Qué: /simulador ya publicaba desde seo-022 (2026-08-30) las dos tablas completas de coeficientes reductores con enlace dofollow a boe.es; lo que faltaba de seo-020 era el enlace visible al simulador oficial de la Seguridad Social y un texto explícito de limitaciones. Se añadió únicamente eso, sin duplicar contenido: un bloque "Limitaciones de esta estimación" bajo el aviso legal existente, con lista de lo que la calculadora no hace (historial real de cotizaciones, complementos/mínimos por cónyuge/pluriactividad/regímenes especiales, IRPF) y un enlace destacado a prestaciones.seg-social.es/simulador-servicio/simulador-pension-jubilacion.html (target=_blank rel=noopener, sin nofollow).
...
- Hipótesis: Confirmada parcialmente — el bloque añadido cumple el criterio de éxito; el efecto en tráfico/posición queda por medir a 21 días en Search Console.
- Criterio de éxito: dist/simulador/index.html contiene el enlace dofollow a seg-social.es y el texto de limitaciones; el DISCLAIMER sigue visible; npm run build pasa. Cumplido.
- Métrica y plazo: `node scripts/auditar-money-set.mjs` tras el build → /simulador con 2269 palabras únicas en `<main>` (umbral 1200), sin noindex. El único fallo que sigue reportando ese script (canibalización /simulador vs como-interpretar-simulador-jubilacion) es preexistente y no depende de esta tarea.
- Riesgo identificado: la URL de seg-social.es no se pudo verificar con una petición en vivo por el proxy de red del entorno del agente; conviene confirmarla manualmente tras el despliegue.
- Commit: (pendiente, ver informe del día)
- Veredicto del CEO: pendiente
