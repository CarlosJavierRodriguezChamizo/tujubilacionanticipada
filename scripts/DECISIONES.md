...
## 2026-08-31 — seo-019 Renderizar en /simulador una tabla estática de escenarios precalculados (seo)
- Archivos: src/pages/simulador.astro
- Qué: Tabla estática HTML de 50 filas de escenarios (5 edades actuales × 5 tramos de años cotizados × 2 tramos de base reguladora) renderizada en build llamando a calcularEscenario()/edadLegalJubilacion() de src/lib/pension-calculo.ts (mismo motor que la isla React Simulador.jsx), como mejora progresiva bajo la isla, no sustitutiva. El DISCLAIMER de src/consts.ts pasa a ser visible fuera de la isla React (antes solo aparecía tras enviar el formulario, es decir nunca sin JS). webApplicationSchema() cambia requiresJs a false, alineando el JSON-LD WebApplication con el HTML ahora funcional sin JavaScript.
- Por qué: Toda la página dependía de una isla React inerte sin JavaScript; no había ninguna estimación ni el aviso legal visibles para un usuario sin JS, incumpliendo la restricción permanente "el sitio debe funcionar sin JavaScript".
- Hipótesis: Con la tabla estática, <main> pasa de tener pocas palabras únicas indexables a un documento sustancialmente más largo y un usuario sin JS puede leer una estimación para su caso.
- Criterio de éxito: dist/simulador/index.html contiene ≥40 filas de escenario generadas en build (50 obtenidas); la tabla y el DISCLAIMER siguen presentes al simular la eliminación de la isla React del HTML; auditar-money-set.mjs reporta ≥1.200 palabras únicas en <main>. Cumplido.
- Métrica y plazo: scripts/auditar-money-set.mjs sobre /simulador: 893 → 2053 palabras únicas en <main> (umbral 1.200 superado). Sin datos normativos nuevos: la tabla reutiliza exclusivamente constantes y funciones ya citadas contra el BOE en pension-calculo.ts.
- Riesgo: auditar-money-set.mjs sigue devolviendo código de salida 1 por una canibalización preexistente entre "simulador jubilacion" en /simulador y /blog/como-interpretar-simulador-jubilacion, ajena a esta tarea (queda para otra tarea del backlog).
